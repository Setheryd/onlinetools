...
const ToolsPageClient = ({ builtTools, unbuiltTools, categories }) => {
  ...
  const filteredActive = useMemo(() => builtTools.filter(matchesFilters), [builtTools, searchQuery, selectedCategories]);
  const filteredComingSoon = useMemo(() => unbuiltTools.filter(matchesFilters), [unbuiltTools, searchQuery, selectedCategories]);
