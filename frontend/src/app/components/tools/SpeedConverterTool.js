...
const UNITS = [
  { id: 'mps', name: 'Meters/second (m/s)', toMps: 1 },
  { id: 'kmh', name: 'Kilometers/hour (km/h)', toMps: 1000/3600 },
  { id: 'mph', name: 'Miles/hour (mph)', toMps: 1609.344/3600 },
  { id: 'knot', name: 'Knots (kn)', toMps: 1852/3600 },
  { id: 'fps', name: 'Feet/second (ft/s)', toMps: 0.3048 },
];

function convertSpeed(value, fromId, toId) {
...
const SpeedConverterTool = () => {
  ...
  const output = useMemo(() => {
    ...
    const res = convertSpeed(Number(value), fromUnit, toUnit);
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-md border border-gray-200">
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => loadExample(100, 'kmh', 'mph')}>100 km/h → mph</Button>
          <Button variant="outline" size="sm" onClick={() => loadExample(60, 'mph', 'kmh')}>60 mph → km/h</Button>
          <Button variant="outline" size="sm" onClick={() => loadExample(10, 'mps', 'kmh')}>10 m/s → km/h</Button>
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
        <h3 className="text-sm font-semibold text-blue-800 mb-2">About Speed Conversion</h3>
        <p className="text-sm text-blue-700">Conversions are based on exact unit factors (e.g., 1 mph = 1.609344 km/h) and computed entirely in your browser.</p>
      </div>
    </div>
  );
};
