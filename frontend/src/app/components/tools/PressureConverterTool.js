...
const UNITS = [
  ...
  { id: 'mpa', name: 'Megapascal (MPa)', toPa: 1_000_000 },
  { id: 'bar', name: 'Bar', toPa: 100000 },
  { id: 'mbar', name: 'Millibar (mbar)', toPa: 100 },
  { id: 'atm', name: 'Atmosphere (atm)', toPa: 101325 },
  { id: 'psi', name: 'Pounds/in² (psi)', toPa: 6894.757293168361 },
];

function convertPressure(value, fromId, toId) {
  const from = UNITS.find(u => u.id === fromId);
  const to = UNITS.find(u => u.id === toId);
  if (!from || !to) return NaN;
  const pascals = value * from.toPa;
  return pascals / to.toPa;
}

const PressureConverterTool = () => {
  ...
  const output = useMemo(() => {
    ...
    const res = convertPressure(Number(value), fromUnit, toUnit);
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-md border border-gray-200">
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600">Convert between Pa, kPa, MPa, bar, atm, and psi.</p>
          </div>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => loadExample(30, 'psi', 'bar')}>30 psi → bar</Button>
          <Button variant="outline" size="sm" onClick={() => loadExample(1, 'atm', 'kpa')}>1 atm → kPa</Button>
          <Button variant="outline" size="sm" onClick={() => loadExample(200, 'kpa', 'psi')}>200 kPa → psi</Button>
        </div>
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          ...
          <input type="number" value={value} onChange={(e) => setValue(e.target.value)} placeholder="e.g. 30" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-900" />
        </div>
        <div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
        <h3 className="text-sm font-semibold text-blue-800 mb-2">About Pressure Conversion</h3>
        <p className="text-sm text-blue-700">Uses standard pressure units and conversion factors; runs entirely in your browser.</p>
      </div>
    </div>
  );
};

export default PressureConverterTool;
