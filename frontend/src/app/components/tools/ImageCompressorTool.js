...
const ImageCompressorTool = () => {
	...
	const [queue, setQueue] = useState([]); // { id, file, name, size, originalUrl, status, outputUrl, outputBlob }
  ...
  const imgRef = useRef(null);
	...
	useEffect(() => {
		...
		// Build queue (cap at 3 visible/processable items)
		const now = Date.now();
	...
	const processItem = async (itemIndex) => {
		...
		try {
			...
			setQueue((q) => q.map((it, idx) => idx === itemIndex ? { ...it, status: 'error', error: 'Failed to process' } : it));
		} finally {
			// no-op
		}
	};
  ...
  const formatBytes = (bytes) => {
    ...
    const a = document.createElement('a');
		const base = ((queue[selectedIdx]?.name) || 'image').replace(/\.[^.]+$/, '');
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-xl shadow-md border border-gray-200">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Image Compressor & Converter</h1>
        <p className="text-gray-600">High-quality resize/convert with AVIF/WebP/PNG/JPEG via serverless processing.</p>
        <div className="mt-2 p-3 rounded border border-yellow-200 bg-yellow-50 text-yellow-800 text-sm">
          Limit: process up to 3 images at a time. Only the first 3 selected will be queued.
        </div>
      </div>
      ...
      >
        ...
        <div className="mt-2 text-xs text-gray-500">Up to 3 images per batch will be added to the queue.</div>
      </div>
          ...
          <div className="p-3 bg-gray-50 border rounded">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm text-gray-700">Original</div>
					<div className="text-xs text-gray-500">{queue[selectedIdx] ? `${formatBytes(queue[selectedIdx].size)} • ${queue[selectedIdx].name}` : ''}{origDims.w ? ` • ${origDims.w}×${origDims.h}` : ''}</div>
            </div>
          ...
          <div className="p-3 bg-gray-50 border rounded">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-xs text-gray-500">
						{queue[selectedIdx]?.outputBlob ? `${formatBytes(queue[selectedIdx].outputBlob.size)}` : ''}
                {outDims.w ? ` • ${outDims.w}×${outDims.h}` : ''}
						{queue[selectedIdx]?.outputBlob && queue[selectedIdx] && queue[selectedIdx].outputBlob.size < queue[selectedIdx].size ? ` • −${((1 - queue[selectedIdx].outputBlob.size / queue[selectedIdx].size) * 100).toFixed(1)}%` : ''}
              </div>
            </div>
